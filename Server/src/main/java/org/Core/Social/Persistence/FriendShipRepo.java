...
public interface FriendShipRepo extends JpaRepository<FriendShip,String> {
    @Query("SELECT Count(f) > 0 FROM FriendShip_Request f WHERE (f.userOne.id=:userOneId AND f.userTwo.id=:userTwoId) OR (f.userOne.id=:userTwoId OR f.userTwo.id=:userOneId)")
    boolean doesFriendShipExists(@Param("userOneId") String userOneId, @Param("userTwoId") String userTwoId);
    @Query("SELECT f FROM FriendShip_Request f WHERE (f.userOne.id=:userOneId AND f.userTwo.id=:userTwoId) OR (f.userOne.id=:userTwoId OR f.userTwo.id=:userOneId)")
